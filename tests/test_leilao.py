...
class TestLeilao(TestCase):
    ...
    def test_lance_ordem_crescente(self):
    ...
    def test_nao_deve_permitir_propor_um_lance_em_ordem_descrecente(self):
    ...
    def test_deve_retornar_o_valor_para_o_maior_e_menor_lance_quando_leilao_tiver_tres_ou_mais_lances(self):

        with self.assertRaises(LanceInvalido):
            yuri = Usuario('Yuri', 500.0)
            nat = Usuario('Natalia', 500.0)

            lance_do_yuri = Lance(yuri, 100.0)
            lance_da_nat = Lance(nat, 200.0)

            self.leilao.propoe(self.lance_do_gui)
            self.leilao.propoe(lance_do_yuri)
            self.leilao.propoe(lance_da_nat)
